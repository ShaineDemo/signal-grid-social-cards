...
不同 Agent 宿主对视觉规范的执行力度不同，因此 BriefGrid 把可机器检查的[跨宿主输出契约](references/portable-contract.md)与任务级故事、艺术指导预案结合起来。生成的 HTML 必须声明选题主体、标题强调角色、封面到第 2 页的交接问题、封面证据的事实 ID、识别素材来源、页面语法和完整数字信息单元。`scripts/render.cjs` 会先调用 `scripts/audit.cjs`，存在强制规则错误时不会继续出图。
