...
description: Create Chinese or English 3:4 Xiaohongshu/RedNote carousels with publish-ready titles and post copy in a modular modernist “signal grid” style. Use for explainers, product updates, launch summaries, trend cards, and visual news briefs that may benefit from sourced company logos, product images, or interview portraits; do not use for photo-led lifestyle posts or ornate editorial layouts.
---

# Signal Grid Social Cards

Turn a topic or source into an accurate, swipeable visual argument for Xiaohongshu/RedNote. The output is editable HTML/CSS plus ordered 1080×1440 PNG cards and publish-ready copy. Support both Simplified Chinese and natural editorial English.
...
1. Determine the audience, output language, desired number of cards, and any exact title or source material. The primary platform is Xiaohongshu/RedNote. If unspecified, default to 6 Simplified Chinese RedNote cards at 1080×1440. When the user asks for English, produce the complete card set and post package in English rather than translating fragments inside an otherwise Chinese package.
...
9. Render with `scripts/render.cjs <index.html> <output-dir>`. If `playwright` is not locally resolvable, set `NODE_PATH` to the available workspace Node modules directory.
...
- Preserve at least 64 px outer safe margin and 24 px gaps. Keep body text at least 30 px at 1080×1440.
